...
- **Saturation** — 0 % (grayscale) to 100 % (full), encoded directly into the SurfaceFlinger color matrix
...
Temperature uses the **Tanner Helland blackbody approximation** for the warm side (1000–6500 K), normalized so that 6500 K (D65) produces an exact identity matrix. For the cool side (6500–10 000 K), the same Tanner Helland formula is used but with the deviation from neutral amplified by a factor of 2.5×, preserving the correct blue hue (R < G < B at high colour temperatures) that a simple linear R-reduction cannot achieve. The temperature slider uses a log scale in both directions from the 6500 K centre, giving finer control near neutral and coarser steps at extremes.
...
True gamma correction (`output = input^γ`) is a power curve — inherently non-linear — and cannot be represented exactly by a linear 3 × 4 color matrix. The app uses a single linear approximation across the full range:

**scale = 2^(1 − γ)**, applied uniformly to all channels.

This gives the exact correct midtone: at input = 0.5, output = 2^(1−γ) × 0.5 = 0.5^γ. Black is preserved (0 × scale = 0). For darkening (γ > 1), scale < 1 so highlights compress toward black; for brightening (γ < 1), scale > 1 so highlights clip toward white. Neither endpoint is perfectly fixed by a single linear scale, but the midtone behaviour matches a true power curve exactly, and the endpoint drift is mild within the practical range.

The slider runs from γ = 4.0 (darkest, left) through γ = 1.0 (neutral/linear, centre) to γ = 0.1 (brightest, right), on a log scale so equal slider travel produces equal perceptual steps. The default is γ = 2.2, which applies the standard sRGB darkening correction — useful when the display's `ColorMode::NATIVE` output is close to linear and content is authored for sRGB (γ = 2.2) displays. Key reference points: 2.2 = sRGB, 1.8 = ProPhoto, 1.0 = linear (no correction).
...
The service sends a single `su -c 'service call SurfaceFlinger 1015 ...'` shell command per update. One root shell spawn per change; zero background polling or watchdog loops.
